...
export function PipelineBuilder() {
  ...
  const createPipeline = () => {
    ...
    addPipeline({ id: crypto.randomUUID(), name: `Pipeline: ${activeTable}`, sourceTable: activeTable, steps: [] });
  };
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <SearchSelect
            ...
            options={pipelines.filter((p) => p.sourceTable === activeTable).map((p) => ({ value: p.id, label: p.name, sublabel: `${p.steps.length} steps` }))}
            placeholder="Select pipeline..."
            size="md"
          />
        </div>
        <button onClick={createPipeline} className="btn-primary">New Pipeline</button>
      </div>
              ...
              {activePipeline.steps.map((step, index) => {
                ...
                return (
                  <div key={index} className="flex gap-4 animate-fade-in">
                    ...
                    {/* Step card */}
                    <div className="flex-1 rounded-xl transition-all duration-200 hover:border-white/[0.08] relative"
                      style={{ background: "linear-gradient(135deg, rgba(20,20,28,0.9) 0%, rgba(14,14,20,0.95) 100%)", border: "1px solid rgba(255,255,255,0.05)" }}>
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-1.5 h-5 rounded-full" style={{ background: meta.color }} />
                          <div>
                            <div className="text-[13px] font-semibold text-gray-200">{meta.label}</div>
                            <div className="text-[10px] text-gray-500">{meta.desc}</div>
                          </div>
                        </div>
                        <button onClick={() => removeStep(activePipeline.id, index)} className="text-gray-600 hover:text-red-400 transition p-1.5 rounded-md hover:bg-white/[0.04]">
